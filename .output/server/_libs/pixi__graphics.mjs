import { Gt as Rectangle, Ht as Point, It as Circle, Kt as RoundedRectangle, Ln as WRAP_MODES, Rt as Ellipse, Ut as Polygon, Vt as PI_2, ct as Texture, g as BatchGeometry, h as BatchDrawCall, i as Container, it as State, m as BaseTexture, on as premultiplyTint, pt as UniformGroup, qt as SHAPES, r as Bounds, rn as import_earcut, tn as hex2rgb, tt as Shader, wn as DRAW_MODES, x as BatchTextureArray, yn as BLEND_MODES, zt as Matrix } from "./@pixi/accessibility+[...].mjs";
...
function buildNonNativeLine(graphicsData, graphicsGeometry) {
	...
	for (var i = 1; i < length - 1; ++i) {
		...
		if (pdist <= smallerInsideSegmentSq + insideWeight * insideWeight * widthSquared) if (style.join === LINE_JOIN.BEVEL || pdist / widthSquared > miterLimitSquared) {
			if (clockwise) {
				verts.push(imx, imy);
				verts.push(x1 + perpx * outerWeight, y1 + perpy * outerWeight);
				verts.push(imx, imy);
			...
			indexCount += 2;
		} else if (style.join === LINE_JOIN.ROUND) if (clockwise) {
			verts.push(imx, imy);
			verts.push(x1 + perpx * outerWeight, y1 + perpy * outerWeight);
			...
			verts.push(x1 - perpx * innerWeight, y1 - perpy * innerWeight);
			verts.push(x1 + perpx * outerWeight, y1 + perpy * outerWeight);
			if (style.join === LINE_JOIN.ROUND) if (clockwise) indexCount += round(x1, y1, x1 + perpx * outerWeight, y1 + perpy * outerWeight, x1 + perp1x * outerWeight, y1 + perp1y * outerWeight, verts, true) + 2;
...
var GraphicsGeometry = function(_super) {
	...
	*/
	GraphicsGeometry.prototype.addColors = function(colors, color, alpha, size, offset) {
		...
		var rgba = premultiplyTint((color >> 16) + (color & 65280) + ((color & 255) << 16), alpha);
...
var Graphics = function(_super) {
	...
	*/
	Graphics.prototype.finishPoly = function() {
		if (this.currentPath) if (this.currentPath.points.length > 2) {
			this.drawShape(this.currentPath);
...
//#endregion
export { GraphicsGeometry as a, LineStyle as c, GraphicsData as i, graphicsUtils as l, GRAPHICS_CURVES as n, LINE_CAP as o, Graphics as r, LINE_JOIN as s, FillStyle as t };
